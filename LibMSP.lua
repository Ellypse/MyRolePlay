...
function msp:Request( player, fields )
	-- This will often get "Player-RealmName" requests post-5.4.7. Checking
	-- this here removes the need for the chat message filtering.
	if player:gsub("-.+", "") == UNKNOWN then
		return false
	else
		local now = GetTime()
...
-- SendAddonMessage() spits out visible error messages when attempting to send
-- to unreachable units (typically offline or opposite faction). Unfortunately
-- there is no way to stop these at a higher level without requiring a unit
-- rather than a name, so they instead need to be filtered out.
ChatFrame_AddMessageEventFilter("CHAT_MSG_SYSTEM", function(self, event, msg)
	local player = msg:match(ERR_CHAT_PLAYER_NOT_FOUND_S:format("(.+)"))
